...
@app.get("/fhapi/controls/stop/{idr}")
...
@app.get("/fhapi/controls/restart/{idr}")
...
@app.get('/fhapi/commons/bot/restart')
...
@app.get('/fhapi/commons/bot/start')
async def botstart(key: str = None):
    if key != 'incorrectpasswordoops':
        return {"error": [401, "Auth key doesn't match"]}
...
@app.get('/fhapi/commons/bot/stop')
async def botstop(key: str = None):
    if key != 'incorrectpasswordoops':
        return {"error": [401, "Auth key doesn't match"]}
    
    subprocess.run(['sudo', 'systemctl', 'stop', 'falconhostbot'])
